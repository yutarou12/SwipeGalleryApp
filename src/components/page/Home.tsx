...
function LabelSelector({open, folder, onClose, labelOptions}: {open: boolean; folder: string; onClose: () => void; labelOptions: string[];}) {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8787/api/labels/${folder}`)
      .then(async r => {
          ...
          <Button
            ...
            onClick={async() => {
              try {
                ...
                const r = await fetch(`http://localhost:8787/api/labels/${folder}`, {
                  method: "PUT",
...
function SettingBox({ open, onClose, onSaved }: { open: boolean; onClose: () => void; onSaved: (options: string[]) => void }) {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8787/api/label-options')
      .then(async r => {
        if (!r.ok) throw new Error(`GET http://localhost:8787/api/label-options failed: ${r.status}`)
        return r.json()
      })
      .then((data: unknown) => {
  ...
  async function saveLabelOptions() {
    ...
    setMessage(null)
    try {
      const r = await fetch('http://localhost:8787/api/label-options', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ options: labelOptions })
      })
      if (!r.ok) {
        throw new Error(`PUT http://localhost:8787/api/label-options failed: ${r.status}`)
      }
      setMessage('保存しました。')
